...
@router.get("/releases")
async def list_releases(request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)):
    releases = list(session.scalars(select(Release).order_by(Release.created_at.desc())))
    return templates.TemplateResponse(request, "releases/list.html", {**base_context(user, "releases"), "releases": releases})


@router.get("/releases/new")
async def new_release_form(
    request: Request, response: Response, project_id: str | None = None,
    ...
    return templates.TemplateResponse(
        request, "releases/new.html",
        {**base_context(user, "releases"), "projects": projects, "release_types": list(ReleaseType), "project_id": project_id, "csrf_token": csrf_token},
    )


@router.post("/releases/new")
async def create_release(request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)):
    ...
    release = Release(
        ...
        release_type=ReleaseType(form["release_type"]),
...
async def release_detail(release_id: str, request: Request, response: Response, session: Session = Depends(get_session), user: User = Depends(require_login)):
    release = session.get(Release, uuid.UUID(release_id))
    if release is None:
        return RedirectResponse("/releases", status_code=303)
    results = list(
        session.scalars(
            select(ChecklistResult).where(ChecklistResult.release_id == release.id).order_by(ChecklistResult.evaluated_at)
        )
    )
    ...
    csrf_token = get_csrf_token(request)
    return templates.TemplateResponse(
        request, "releases/detail.html",
        {**base_context(user, "releases"), "release": release, "project": project, "grouped": grouped, "csrf_token": csrf_token},
    )


@router.post("/releases/{release_id}/evaluate")
async def re_evaluate(release_id: str, request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)):
    release = session.get(Release, uuid.UUID(release_id))
    form = await request.form()
...
async def waive_checklist_result(result_id: str, request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)):
    result = session.get(ChecklistResult, uuid.UUID(result_id))
    form = await request.form()
    if result and verify_csrf(request, form.get("csrf_token")):
        waive_result(session, result, user_id=user.id, reason=str(form.get("reason") or "Waived by producer."))
        from produceros.services.checklist import summarize_status

        release = session.get(Release, result.release_id)
        all_results = list(session.scalars(select(ChecklistResult).where(ChecklistResult.release_id == release.id)))
