...
async def start_pairing_route(
    ...
    lan_ip = detect_private_ipv4()
    pair_url = (
        f"http://{lan_ip}:{settings.port}/lan/pair/{device.id}?code={code}"
        if lan_ip
        else f"/lan/pair/{device.id}?code={code}"
    )
...
async def pair_confirm_form(
    ...
    return templates.TemplateResponse(
        request,
        "lan/pair.html",
        {"device_id": device_id, "code": code, "csrf_token": csrf_token, "error": None},
    )


@router.post("/lan/pair/{device_id}/confirm")
async def pair_confirm_submit(
    device_id: str, request: Request, response: Response, session: Session = Depends(get_session)
):
    settings = get_settings()
    form = await request.form()
    csrf_token = get_csrf_token(request)
    ...
    try:
        device, token = pairing_service.confirm_pairing(
            ...
            max_attempts_per_minute=settings.pairing_rate_limit_per_minute,
        )
    ...
    redirect.set_cookie(
        pairing_service.DEVICE_COOKIE_NAME,
        device_cookie,
        httponly=True,
        samesite="lax",
        secure=request.url.scheme == "https",
        max_age=pairing_service.SESSION_LIFETIME_DAYS * 86400,
    )
    return redirect
