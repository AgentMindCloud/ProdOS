...
from produceros.marketing.campaigns import (
    create_campaign,
    create_content_asset,
    list_campaigns,
    list_content_assets,
)
from produceros.marketing.engine import (
    archive_draft,
    edit_draft,
    export_draft_as_markdown,
    generate_draft,
)
...
async def marketing_home(
    ...
    drafts = list(
        session.scalars(select(MarketingDraft).order_by(MarketingDraft.created_at.desc()).limit(30))
    )
    ...
    return templates.TemplateResponse(
        ...
        "marketing/index.html",
        {
            **base_context(user, "marketing"),
            ...
            "draft_types": list(MarketingDraftType),
            "content_types": list(ContentAssetType),
...
async def new_campaign(
    ...
    if verify_csrf(request, form.get("csrf_token")) and form.get("name"):
        create_campaign(
            session,
            name=str(form["name"]),
            campaign_type=CampaignType(str(form.get("campaign_type") or "custom")),
...
async def new_content_asset(
    ...
    if (
        ...
        and form.get("title")
        and form.get("content_type")
    ):
        create_content_asset(
            session,
            title=str(form["title"]),
            content_type=ContentAssetType(str(form["content_type"])),
            project_id=uuid.UUID(str(form["project_id"])) if form.get("project_id") else None,
        )
    return RedirectResponse("/marketing", status_code=303)
...
async def generate_marketing_draft(
    request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)
):
    form = await request.form()
    if (
        ...
        and form.get("draft_type")
    ):
        project = session.get(Project, uuid.UUID(str(form["project_id"])))
        if project:
            draft = generate_draft(
                session,
                draft_type=MarketingDraftType(str(form["draft_type"])),
                project=project,
                user_id=user.id,
            )
...
async def draft_detail(
    ...
    return templates.TemplateResponse(
        request,
        "marketing/draft_detail.html",
        {**base_context(user, "marketing"), "draft": draft, "csrf_token": csrf_token},
    )


@router.post("/marketing/drafts/{draft_id}/edit")
async def edit_marketing_draft(
    draft_id: str,
    request: Request,
    session: Session = Depends(get_session),
    user: User = Depends(require_login),
...
async def archive_marketing_draft(
    draft_id: str,
    request: Request,
    session: Session = Depends(get_session),
    user: User = Depends(require_login),
...
async def export_marketing_draft(
    draft_id: str, session: Session = Depends(get_session), user: User = Depends(require_login)
):
    draft = session.get(MarketingDraft, uuid.UUID(draft_id))
    if draft is None:
        return RedirectResponse("/marketing", status_code=303)
