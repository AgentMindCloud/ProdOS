...
from produceros.services.search import delete_saved_filter, global_search, list_saved_filters, save_filter
...
@router.get("/search")
async def search_page(request: Request, response: Response, q: str = "", session: Session = Depends(get_session), user: User = Depends(require_login)):
    results = global_search(session, q) if q.strip() else {"projects": [], "artists": [], "releases": []}
    saved_filters = list_saved_filters(session)
    csrf_token = get_csrf_token(request)
    return templates.TemplateResponse(
        request, "search/index.html",
        {**base_context(user, ""), "query": q, "results": results, "saved_filters": saved_filters, "csrf_token": csrf_token},
    )


@router.post("/search/saved-filters/new")
async def new_saved_filter(request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)):
...
async def delete_saved_filter_route(key: str, request: Request, session: Session = Depends(get_session), user: User = Depends(require_login)):
    form = await request.form()
