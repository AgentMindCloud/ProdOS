...
async def delivery_home(
    ...
    packages = list(
        session.scalars(select(DeliveryPackage).order_by(DeliveryPackage.created_at.desc()))
    )
...
async def new_package(
    ...
    if (
        ...
        and form.get("preset_id")
        and form.get("output_directory")
    ):
        ...
        if project and preset:
            package = create_package(
                ...
                project=project,
                preset=preset,
                name=str(form.get("name") or f"{project.working_title} -- {preset.name}"),
                output_directory=str(form["output_directory"]),
                user_id=user.id,
            )
...
async def approve_package_route(
    package_id: str,
    request: Request,
    session: Session = Depends(get_session),
    user: User = Depends(require_login),
...
async def execute_package_route(
    package_id: str,
    request: Request,
    session: Session = Depends(get_session),
    user: User = Depends(require_login),
    ...
    if package and verify_csrf(request, form.get("csrf_token")):
        try:
            ...
            return RedirectResponse(
                f"/delivery/packages/{package_id}?error={quote(str(exc))}", status_code=303
            )
    return RedirectResponse(f"/delivery/packages/{package_id}", status_code=303)
