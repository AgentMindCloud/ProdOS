...
def test_project_creation_and_state_change_are_audited(db_session):
    ...
    catalog_service.change_project_state(
        db_session, project, ProjectState.MIX, note="Ready for mixing"
    )
...
def test_login_events_are_audited(client):
    ...
    try:
        events = list(
            session.scalars(select(AuditEvent).where(AuditEvent.event_type == "auth.login_failed"))
        )
        assert len(events) >= 1
    finally:
        session.close()
...
def test_audit_events_never_contain_password(db_session):
    ...
    create_first_admin(
        db_session, username="someone", password="super-secret-password-123", display_name="Someone"
    )
