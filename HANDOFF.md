...
- Hostinger update is live and verified. The owner uploaded/extracted the ZIP
  into nested `public_html/public_html`; the agent copied only the six changed
  download/page files to the outer live directory. Existing unrelated files and
  nested deployment copies were retained. Live homepage shows 0.2.1, demo edits
  and reset work, and all six changed public files match the build hashes.
  The full 28,211,113-byte HTTPS app download matched the reviewed SHA-256 and
  passed ZIP integrity. CSP, nosniff, DENY framing and no-cache HTML headers
  were also observed on the live server. File Manager access can still be intermittent.
  Deployment: `release-artifacts/prodos-site-20260910.zip`
